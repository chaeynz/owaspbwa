...
sub acevm_included_player_newer {
  return 156735 gt int($gConfig->get('product.buildNumber', ''));
}
...
sub install_content_tools {
  ...
  # Setuid root
  if (vmware_product() eq 'tools-for-freebsd') {
    safe_chmod(04555, $answer . '/bin32-6/vmware-user-suid-wrapper');
    safe_chmod(04555, $answer . '/bin32-53/vmware-user-suid-wrapper');
    safe_chmod(04555, $answer . '/bin64-6/vmware-user-suid-wrapper');
    safe_chmod(04555, $answer . '/bin64-53/vmware-user-suid-wrapper');
  } elsif (vmware_product() eq 'tools-for-solaris') {
    ...
    # note: vmware-hgfsounter is only currently found in tools-for-linux
   ...
   if ((vmware_product() eq 'tools-for-linux') &&
      ...
      if (open(MOUNTS, '</proc/mounts')) {
         ...
         close(MOUNTS);
            ...
            (-x internal_which('cdcontrol'))) {
      $eject_cmd = internal_which('cdcontrol') . " eject";
      my @mountlines = split('\n', direct_command(shell_string($gHelper{'mount'})));
      foreach my $mountline (@mountlines) {
         chomp($mountline);
         $device = $mountline;
         $device =~ s/(.*)\ on\ (.*)\ \((.*?,)\ (.*)$/$1/;
         $mountpoint = $mountline;
         $mountpoint =~ s/(.*)\ on\ (.*)\ \((.*?,)\ (.*)$/$2/;
         $fstype = $mountline;
         $fstype =~ s/(.*)\ on\ (.*)\ \((.*?,)\ (.*)$/$3/;

         # Perhaps it would be better to get rid of the comma below with a
         # better regex above.
   ...
   foreach my $candidate_mount (@candidate_mounts) {
      ...
      if (check_mountpoint_for_tools($mountpoint)) {
         ...
         if (vmware_product() eq 'tools-for-freebsd') {
            ...
            direct_command(shell_string($gHelper{'umount'}) .
                           ' "' . $device . '"');
         }
...
sub install_content_viperl {
   ...
   foreach my $line (direct_command("ls /lib/libssl.* /usr/lib/libssl.* 2> /dev/null")) {
      if ($line =~ /(0\.9\.8])/) {
         if (! defined $ssl_version || $1 gt $ssl_version) {
            $ssl_version = $1;
         }
      }
   }
   ...
   if (! defined $ssl_version) {
      print wrap("OpenSSL 0.9.8 was not found on your system.  OpenSSL is required " .
                 "for encrypted connections.\n\n", 0);
   }

   my @modules = (
     {'module' => 'Crypt::SSLeay',         'version' => '0.55',   'path' => "Crypt-SSLeay-0.55-$ssl_version"},
     ...
     {'module' => 'IO::Compress::Zlib',    'version' => '2.005',  'path' => 'IO-Compress-Zlib-2.005'},
     {'module' => 'Compress::Raw::Zlib',   'version' => '2.005',  'path' => 'Compress-Raw-Zlib-2.005'},
     {'module' => 'Archive::Zip',          'version' => '1.20',   'path' => 'Archive-Zip-1.20'},
     {'module' => 'Data::Dumper',          'version' => '2.102',  'path' => 'Data-Dumper-2.121'},
     {'module' => 'Class::MethodMaker',    'version' => '2.08',   'path' => 'Class-MethodMaker-2.10'},
     {'module' => 'XML::LibXML',           'version' => '1.58',   'path' => 'XML-LibXML-1.63'},
     {'module' => 'LWP',                   'version' => '5.8.05', 'path' => 'libwww-perl-5.805'},
...
sub finalize_vix_install {
  ...
  if (-e $vixAltConfig && defined(block_match($vixAltConfig, $vixLibdir))) {
    # Remove any pre-existing values for 'vix.libdir'
    block_restore($vixAltConfig, $cVixMarkerBegin, $cVixMarkerEnd);
  } elsif (!-e $vixAltConfig) {
    if (!-d $vixAltDir) {
      create_dir($vixAltDir, 0x1);
    }
    # block_append() won't create a file so create one here.
    system('/bin/touch' . ' ' . $vixAltConfig);
  }
...
# and returning 0 if the installed product can coexist, or 1 if it
# must be removed (as in for an up/downgrade or conflicting product),
# also may error() out if the user cancels the install here
#
sub installed_product_needs_removal {
...
#      installed_product_needs_removal)
#
...
sub init_product_arch_hash {
  ...
  $multi_arch_products{'rcli'} = 1;
  $multi_arch_products{'viperl'} = 1;
}
...
# Look to see if this vix package built under PRODUCT=ws or PRODUCT=wgs.
# Return "ws" if the workstation vix lib dir "Workstation-6.5.0" can be
# found, undef otherwise.
sub vmware_vix_type {
   my $product_libdir = "Workstation-6.5.0";

   if (file_name_exist('vmware-vix/lib/' . $product_libdir)) {
      return "ws";
   }
   return undef; 
}
...
sub main {
  ...
  if (internal_basename($0) eq $cInstallerFileName) {
    ...
    if ($#ARGV > -1) {
      ...
      while ($#ARGV != -1) {
        ...
        } elsif (lc($arg) =~ /^(-)?(-)?(no-dep-warning)$/) {
          $gOption{'no-dep-warning'} = 1;
        } else {
          install_usage();
        }
      }
   ...
   my $noDepWarning = 'no';
   if (file_name_exist($gInstallerMainDB)) {
      db_load();
      $noDepWarning = db_get_answer_if_exists('NODEPWARNING') ? 'yes' : 'no';
   }

   if (((defined(vmware_vix_type()) && vmware_vix_type() eq 'ws') ||
        vmware_product() eq 'ws') && ! defined($gOption{'no-dep-warning'})) {
	if (get_answer(
           '==================== WARNING ====================' .
           "\n" .
           'This installer is deprecated for Workstation, Player, and VIX.  ' .
           'Chromium beta 2 is scheduled to ship with the Universal Installer.  ' .
           'See http://wiki.eng.vmware.com/PIT/UniversalInstaller for more information.' .
           "\n\n" .
           'If you have any dependencies on the functionality of this installer ' .
           'please e-mail pit@vmware.com to let us know.  If this message broke ' .
           'your automation script that likely means you.' .
           "\n\n" .
           'Please use the new installer unless there is something about it that ' .
           'is blocking you.  If there is, let us know what.' .
	   "\n" .
           '==================== WARNING ====================' .
           "\n\n" .
           'If you wish to avoid this warning on new installs pass --no-dep-warning.' .
           "\n\n" .
           'Do you understand?',
           'yesno', $noDepWarning) eq 'no') {
         exit 1;
      } else {
         $gOption{'no-dep-warning'} = 1;
      }
   ...
   ### End pre-db checks ###
    ...
    if ($gOption{'no-dep-warning'}) {
	db_add_answer('NODEPWARNING', 'yes');
    }
  ...
  if (internal_basename($0) eq $cModuleUpdaterFileName) {
    ...
    if (not (($installed_version eq '7.8.5') and
      ...
      if (vmware_product() eq 'tools-for-linux') {
         if (defined db_get_answer_if_exists('VMHGFS_CONFED') &&
            ...
            if (is_selinux_enabled()) {
               system("/sbin/restorecon /etc/fstab");
            }
         }
         if (defined db_get_answer_if_exists('VMXNET_CONFED') &&
             db_get_answer('VMXNET_CONFED') eq 'yes') {
            # Since the Tools services script no longer unloads vmxnet,
            # the uninstaller must do it.
            kmod_unload('vmxnet', 0);
         }
         if (defined db_get_answer_if_exists('VMXNET3_CONFED') &&
             db_get_answer('VMXNET3_CONFED') eq 'yes') {
            # Since the Tools services script no longer unloads vmxnet3,
            # the uninstaller must do it.
