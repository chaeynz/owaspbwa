...
class SoftwareProperties(object):
  ...
  def add_source_from_line(self, line):
    ...
    """
    (line, file) = expand_ppa_line(line.strip(), self.distro.codename)
    self.check_and_add_key_for_whitelisted_channels(line)
    new_entry = SourceEntry(line, file)
  ...
  def remove_source(self, source):
    ...
    # here
    if source.file != apt_pkg.Config.FindFile("Dir::Etc::sourcelist"):
