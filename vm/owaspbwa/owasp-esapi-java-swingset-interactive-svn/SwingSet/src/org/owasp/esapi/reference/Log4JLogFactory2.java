 ...
 */
public class Log4JLogFactory2 implements LogFactory {
	
	private HashMap<Object, Logger> loggersMap = new HashMap<Object, Logger>();
	
	/**
	* Null argument constructor for this implementation of the LogFactory interface
	* needed for dynamic configuration.
	*/
	public Log4JLogFactory2() {}
	...
	*/
    ...
    private static class Log4JLogger implements org.owasp.esapi.Logger {
		...
		static{ 
			try {
				String logFileName = ESAPI.securityConfiguration().getLogFileName();
				// CD: log file must already exist for this to work!
				String logFilePath = ESAPI.securityConfiguration().getResourceFile(logFileName).getCanonicalPath();
//				FileAppender fA = new FileAppender(new PatternLayout("%-4r [%t] %-5p %c %x - %m%n"), logFilePath);
//				org.apache.log4j.Logger.getRootLogger().addAppender(fA);
				RollingFileAppender rFA = new RollingFileAppender(new PatternLayout("%-4r [%t] %-5p %c %x - %m%n"), logFilePath);
				rFA.setMaxFileSize(""+ESAPI.securityConfiguration().getMaxLogFileSize());
    	...
    	*/
        ...
        private void log(Level level, EventType type, String message, Throwable throwable) {
            ...
            // log the message
            jlogger.log(level, "[" + typeInfo + userInfo + " -> " + appInfo + "] " + clean, throwable);
        }
    	...
    	*/
		...
		// Added for OWASP BWA to fix build
		public void always(Logger.EventType type,
            java.lang.String message,
            java.lang.Throwable throwable) {
			
			throw new UnsupportedOperationException("Log4JLogger.always() not implemented!");
		...
		public int getESAPILevel() {
			...
			throw new UnsupportedOperationException("Log4JLogger.getESAPILevel() not implemented!");
