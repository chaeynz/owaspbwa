 ...
 * <PRE>
 * <p/>
 * accountId | accountName | hashedPassword | roles | locked | enabled | oldPasswordHashes | lastHostAddress | 
 * lastPasswordChangeTime | lastLoginTime | lastFailedLoginTime | expirationTime | failedLoginCount
 * ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 * @since April 2011
 */
public class FileBasedAuthenticator2 implements org.owasp.esapi.Authenticator {
     ...
     *
     */
    public FileBasedAuthenticator2() {
    }
     ...
     * accountId | accountName | hashedPassword | roles | locked | enabled | oldPasswordHashes | lastHostAddress | lastPasswordChangeTime | lastLoginTime | lastFailedLoginTime | expirationTime | failedLoginCount
...
//            writer.println("# accountId | accountName | hashedPassword | roles | locked | enabled | csrfToken | oldPasswordHashes | lastPasswordChangeTime | lastLoginTime | lastFailedLoginTime | expirationTime | failedLoginCount");
    ...
    private String save(DefaultUser user) {
        ...
        sb.append(user.getAccountId()); // 0
        sb.append(" | ");
        sb.append(user.getAccountName()); // 1
        sb.append(" | ");
        sb.append(getHashedPassword(user)); // 2
        sb.append(" | ");
        sb.append(dump(user.getRoles())); // 3
        sb.append(" | ");
        sb.append(user.isLocked() ? "locked" : "unlocked"); // 4
        sb.append(" | ");
        sb.append(user.isEnabled() ? "enabled" : "disabled"); // 5
        sb.append(" | ");
        sb.append(dump(getOldPasswordHashes(user))); // 6
        sb.append(" | ");
        sb.append(user.getLastHostAddress()); // 7
        sb.append(" | ");
        sb.append(user.getLastPasswordChangeTime().getTime()); // 8
        sb.append(" | ");
        sb.append(user.getLastLoginTime().getTime()); // 9
        sb.append(" | ");
        sb.append(user.getLastFailedLoginTime().getTime()); // 10
        sb.append(" | ");
        sb.append(user.getExpirationTime().getTime()); // 11
        sb.append(" | ");
        sb.append(user.getFailedLoginCount()); // 12
        return sb.toString();
    }
    ...
    public void verifyPasswordStrength(String oldPassword, String newPassword) throws AuthenticationException {
        ...
        if (oldPassword != null) {
            ...
            for (int i = 0; i < length - 2; i++) {
                ...
                if (newPassword.indexOf(sub) > -1) {
                    throw new AuthenticationCredentialsException("New password cannot contain pieces of old password", "New password cannot contain pieces of old password");
                }
            }
        }
        ...
        if (strength < 16) {
            throw new AuthenticationCredentialsException("New password is not long and complex enough", "New password is not long and complex enough");
        }
    }
