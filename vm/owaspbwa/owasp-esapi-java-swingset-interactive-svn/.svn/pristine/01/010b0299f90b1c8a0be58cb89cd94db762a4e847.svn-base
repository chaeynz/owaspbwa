...
//		ESAPI.securityConfiguration().setResourceDirectory( path );
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws IOException {
		...
		try {
			...
			if(session != null){
				try{
					// CP_2012-08-08 Made this modified call so that the login only tries to login
					// from the session or the remember-me token. Prevents double login attempts
					// in the LoginSolution. Only works with FileBasedAuthenticator2
					Authenticator authenticator = ESAPI.authenticator(); 
					if (authenticator instanceof FileBasedAuthenticator2) {
						((FileBasedAuthenticator2)authenticator).login(request, response, true);
					} else {
						authenticator.login();
					}
				}
