...
<%
	...
	try {	
		// CP_2012-07-08 Validate dropdown. Last true is for implicit canonicalization
		String type = request.getParameter("type");
		if (type != null) {
			type = ESAPI.encoder().canonicalize(type);
			type = ESAPI.validator().getValidInput("Swingset Validation Secure Exercise - Validate type dropdown",
					type, "ValidatorTypes", 10, false, true);

			canonical = ESAPI.encoder().canonicalize(input);
			ESAPI.validator().getValidInput("Swingset Validation Secure Exercise", canonical, type, 200, false);
		}
...
<form action="main?function=ValidateUserInput&solution" method="POST">
	...
	<select name="type" size="1">
		<option selected>SafeString</option>
		<option>Email</option>
