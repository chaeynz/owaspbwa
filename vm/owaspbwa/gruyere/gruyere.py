...
def main():
  ...
  #   (3) Run on a random url which contains an idenfier generated randomly so a
  # real attacker won't know it and any attack would be slowed down (and
  # therefore more likely to be noticed) as the attacker tries every
  # possibilities of the random number.
  #   (3) Run on a url containing a random identifier and random path prefix,
  # making it more difficult for an attacker to locate and quit if a request
  # is received without this identifier (except for an empty path which
  # redirects and /favicon.ico).
  ...
  global http_server
  http_server = HTTPServer((server_name, server_port),          # DO NOT CHANGE
                           GruyereRequestHandler)               # DO NOT CHANGE
...
class GruyereRequestHandler(BaseHTTPRequestHandler):
  ...
  def DoGetOrPost(self):
    ...
    #   (4) Inject a random identifier as the first part of the path and
